...
一个完全离线运行的现代化多编辑器桌面应用，专为开发者和文本工作者设计。基于 Electron + Vue 3 构建，无需网络连接，提供强大的 JSON 编辑、文本编辑、代码编辑和历史记录管理功能，所有数据保存在本地。
...
> **Tore 是一款完全离线的桌面应用。**
>
> - 🚫 **无需网络连接** — 所有功能在本地运行，断网环境下照常使用
> - 💾 **数据仅存储在本机** — 使用本地 SQLite 数据库，文件保存在你指定的目录
> - 🛡️ **零数据收集** — 不上传任何数据到云端，不包含遥测、不采集使用统计
> - 🔑 **你的数据你做主** — 没有账号系统，没有云端同步，一切尽在你掌控之中
>
> 在这个越来越多的工具将你的数据上传到未知服务器的时代，Tore 让你完全拥有自己的数据。
...
- JSON 编辑器：.json, .jsonc
- 文本编辑器：.txt, .md, .markdown
- 代码编辑器：.js, .ts, .py, .java, .cpp, .go, .rb, .sql, .xml, .yaml, .toml, .ini, .sh, .dockerfile 等
