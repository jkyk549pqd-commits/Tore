...
export function useClipboardActions({ emit, t, localValue }) {
  ...
  async function handleCopy() {
    if (!localValue.value || localValue.value.trim() === "") {
      ElMessage.warning(t("codeEditor.emptyContent"));
      return;
    }
    ...
    try {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        ...
        ElMessage.success(t("codeEditor.copySuccess"));
      } else {
        ...
        try {
          ...
          if (successful) {
            ...
            ElMessage.error(t("codeEditor.copyFailed"));
          }
        } catch (err) {
          ElMessage.error(t("codeEditor.copyFailed"));
  ...
  async function handlePaste() {
    ...
    if (!navigator.clipboard || !navigator.clipboard.readText) {
      ElMessage.error(t("codeEditor.browserNotSupportPaste"));
      return;
    }
    ...
    try {
      ...
      if (pastedText && pastedText.trim() !== "") {
        ...
        ElMessage.warning(t("codeEditor.clipboardEmpty"));
      }
    } catch (err) {
      if (
        ...
        ElMessage.error(t("codeEditor.noClipboardPermission"));
      } else {
        ElMessage.error(t("codeEditor.pasteFailed"));
      }
