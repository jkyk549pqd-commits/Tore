...
import CodeEditorPanel from "./components/code-editor-panel.vue";
...
// 防止路由信息未加载时的错误
const routeName = computed(() => route.value?.name || "codeEditor");
...
function compareText() {
  try {
    if (
      ...
      ElMessage.warning(t("codeEditor.diffEmptyWarning"));
    }
  } catch (error) {
    console.error("对比失败:", error);
    ElMessage.error(t("codeEditor.diffFailed"));
  }
}
...
function generateFilename() {
  ...
  return `code_${year}${month}${day}_${hour}${minute}${second}.${selectedFormat.value}`;
}
...
async function handleManualSave() {
  ...
  if (!textContent.value || textContent.value.trim() === "") {
    ElMessage.warning(t("codeEditor.emptyContentWarning"));
    return;
  }
  ...
  try {
    ...
    const result = await AutoSaveManager.manualSaveCodeData({
      fromPage: 1, // 来自菜单页
    ...
    if (result.success) {
      ...
      updateSaveState(false);
      ElMessage.error(t("codeEditor.saveFailed"));
    }
  } catch (error) {
