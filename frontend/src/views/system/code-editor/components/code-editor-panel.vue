    ...
    >
      ...
      <div class="char-count-bar">
        <span class="char-count-info">
          {{ t("codeEditor.characters") }}: <strong>{{ charCount }}</strong>
          {{ t("codeEditor.lines") }}: <strong>{{ lineCount }}</strong>
          <span class="selected-info">
            | {{ t("codeEditor.selected") }}:
            <strong>{{ selectedCharCount }}</strong>
          </span>
        </span>
      </div>
      <!-- 导入弹框 -->
      <CodeEditorImportDialog
...
// 新增语言模式导入
// TypeScript 使用 JavaScript 模式，TypeScript 是 JavaScript 的超集
// C 和 C++ 已经通过 clike.js 支持
...
import { useCodeEditorOperations } from "../composables/useCodeEditorOperations";
import CodeEditorButtons from "./code-editor-buttons.vue";
import CodeEditorImportDialog from "./code-editor-import-dialog.vue";
import MarkdownPreview from "./markdown-preview.vue";
import CodeTemplateButton from "./code-template-button.vue";
...
// 使用代码生成器 Composable
const { supportsCodeGeneration, generateCode, getTemplateOptions } =
  useCodeGenerator();
...
// 判断当前是否支持代码生成
const isCodeGeneratorSupported = computed(() => {
  return supportsCodeGeneration(props.currentFileFormat);
});
...
const {
  buttonStates: codeEditorButtonStates,
  handleFormat,
  handleCompress,
  showFormatButtons,
} = useCodeEditorOperations({
  emit,
  t,
  localValue,
    ...
    if (cmInstance.value && newOptions !== oldOptions) {
      ...
      if (newOptions.theme !== oldOptions?.theme && newOptions.theme) {
        try {
          ...
          // console.error("code-editor-panel: 加载主题失败:", error);
        }
      }
      ...
      // console.log(
      //   "code-editor-panel: 跳过配置更新，cmInstance 不存在或配置未变化"
      // );
    }
  },
  { deep: true }
...
function handleCodeGenerated(code) {
  ...
  emit("change", code);
  ElMessage.success(t("codeEditor.codeGenerated") || "代码生成成功");
}
...
function handleDemo() {
  try {
    // 检查当前语言是否支持代码生成
    if (supportsCodeGeneration(props.currentFileFormat)) {
      // 获取当前语言的模板选项
      const templateOptions = getTemplateOptions(props.currentFileFormat);
      ...
      if (templateOptions.length > 0) {
        ...
        // 生成代码
        const demoContent = generateCode(
          props.currentFileFormat,
          randomTemplate.value
        );

        // 更新编辑器内容
        localValue.value = demoContent;
        emit("update:modelValue", demoContent);
        emit("change", demoContent);
      } else {
        throw new Error(`没有找到 ${props.currentFileFormat} 的可用模板`);
      }
    } else {
      // 回退到原有的配置文件生成逻辑
      // console.log("props.currentFileFormat:", props.currentFileFormat);
    ...
    ElMessage.error(
      t("json.editor.generateRandomJsonFailed") || "生成示例内容失败"
    );
  }
}
</script>
