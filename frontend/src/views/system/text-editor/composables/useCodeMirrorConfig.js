...
export function useCodeMirrorConfig() {
   ...
   */
  const STORAGE_KEYS = {
    cmTheme: "textEditor.cmTheme",
    cmFontSize: "textEditor.cmFontSize",
    cmLineWrapping: "textEditor.cmLineWrapping",
    cmLineNumbers: "textEditor.cmLineNumbers",
  };
  ...
  const currentFileFormat = ref(
    appConfigStore.$state.selectedFileFormat || "text"
  );
  ...
  const initializeTheme = () => {
    ...
    try {
      ...
      if (savedTheme && options.find((opt) => opt.value === savedTheme)) {
        // console.log("text-editor: 恢复用户保存的主题:", savedTheme);
        cmTheme.value = savedTheme;
        return;
      }
    } catch (error) {
      console.warn("text-editor: 读取 localStorage 主题偏好失败:", error);
    }

    // 选择第一个可用主题作为 fallback
    // console.log("text-editor: 切换到默认主题:", options[0].value);
  ...
  const initializeEditorConfig = () => {
    ...
    try {
      ...
      if (savedFontSize) {
        ...
        if (!isNaN(fontSize) && fontSize >= 10 && fontSize <= 32) {
          // console.log("text-editor: 恢复用户保存的字体大小:", fontSize);
          cmFontSize.value = fontSize;
        }
      }
    } catch (error) {
      console.warn("text-editor: 读取 localStorage 字体大小失败:", error);
    }
    ...
    try {
      ...
      if (savedLineWrapping !== null) {
        ...
        // console.log("text-editor: 恢复用户保存的行自动换行:", lineWrapping);
        cmLineWrapping.value = lineWrapping;
      }
    } catch (error) {
      console.warn("text-editor: 读取 localStorage 行自动换行失败:", error);
    }
    ...
    try {
      ...
      if (savedLineNumbers !== null) {
        ...
        // console.log("text-editor: 恢复用户保存的行号显示:", lineNumbers);
        cmLineNumbers.value = lineNumbers;
      }
    } catch (error) {
      console.warn("text-editor: 读取 localStorage 行号显示失败:", error);
    }
  };
  ...
  watch(
    ...
    () => {
      console.log(
        "text-editor: CodeMirror 配置变更, 新主题:",
        cmTheme.value,
        "新格式:",
  ...
  const saveThemePreference = (theme) => {
    ...
    try {
      ...
      // console.log("text-editor: 保存主题偏好:", theme);
    } catch (error) {
      console.warn("text-editor: 保存主题偏好到 localStorage 失败:", error);
    }
  };
  ...
  const saveEditorConfig = () => {
    try {
      ...
      // console.log("text-editor: 保存字体大小:", cmFontSize.value);
    } catch (error) {
      console.warn("text-editor: 保存字体大小到 localStorage 失败:", error);
    }
    ...
    try {
      ...
      // console.log("text-editor: 保存行自动换行:", cmLineWrapping.value);
    } catch (error) {
      console.warn("text-editor: 保存行自动换行到 localStorage 失败:", error);
    }
    ...
    try {
      ...
      // console.log("text-editor: 保存行号显示:", cmLineNumbers.value);
    } catch (error) {
      console.warn("text-editor: 保存行号显示到 localStorage 失败:", error);
    }
  };
