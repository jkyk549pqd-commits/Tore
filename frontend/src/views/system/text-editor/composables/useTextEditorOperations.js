...
export function useTextEditorOperations({
  emit,
  t,
  localValue,
  currentFileFormat,
  ...
  // 支持格式化和压缩的格式列表
  const supportedFormats = ["sql", "yaml", "xml", "toml", "conf"];
  ...
  function handleTextOperation(operation, stateKey) {
    ...
    if (!isFormatSupported.value) {
      ElMessage.warning(
        t("textEditor.formatNotSupported") ||
    ...
    if (!textContent || textContent.trim() === "") {
      ElMessage.warning(
        t("textEditor.emptyContentWarning") || "内容为空，无法操作"
      );
      return;
    }
