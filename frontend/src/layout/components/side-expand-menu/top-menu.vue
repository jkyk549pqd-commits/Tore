...
<template>
  <div class="top-menu-container">
    ...
    <!-- 一级菜单展示 -->
    <a-menu class="smart-menu" :selectedKeys="selectedKeys" mode="inline" :theme="theme">
      <template v-for="item in menuTree" :key="item.menuId">
        <template v-if="item.visibleFlag">
          ...
          >
            ...
            {{ menuNameAdapter($t(item.menuName)) }}
          </a-menu-item>
        </template>
      </template>
    </a-menu>
...
function onSelectMenu(menuItem) {
  ...
  if (
    menuItem.menuType === MENU_TYPE_ENUM.MENU.value &&
    (_.isEmpty(menuItem.children) ||
      menuItem.children.every((e) => !e.visibleFlag))
  ) {
